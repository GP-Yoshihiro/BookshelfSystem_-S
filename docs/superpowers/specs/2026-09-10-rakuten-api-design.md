...
**設計変更（2026-09-18）:** 当初は「入力のたびには叩かない」方針だったが、
利用者の要望により入力に応じた候補表示を追加した。レート制限への配慮は
以下の3点で担保する。

1. 入力停止から 400ms 待ってから1回だけ取得する（デバウンス）
2. 2文字未満では取得しない
3. 候補の取得に本検索と同じ `getCachedRakutenSearch` を使う。候補を出した
   時点で結果がキャッシュされるため、続けて検索しても楽天へ再送されない

結果として、1回の検索操作あたりの呼び出しは1〜3回に収まる。

```
[/search 画面]
  利用者がタイトルを入力する
    ↓ 400ms 入力が止まり、2文字以上なら
  suggestBooksAction が候補を最大8件返す（本検索と同じキャッシュを使う）
    ↓ 候補を選ぶ、または「検索」を押す
    ↓
