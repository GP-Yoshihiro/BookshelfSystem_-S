...
**再認証はセキュリティ境界ではない。放置された画面に対する UI ガードである。**

この位置づけを正確に記録しておく。過大評価すると、防げていない脅威を防げていると
誤認したまま機能を追加してしまうため。

### 何が起きているか

`/settings/invites` は Server Component で招待コードを取得し、Client Component
`InviteCodeList` へ props として渡す。Next.js App Router では、Server Component から
Client Component へ渡された props は RSC ペイロード（`self.__next_f.push(...)`）として
シリアライズされ、**ページを開いた時点でブラウザへ送信される**。

`maskCode()` による伏せ字は描画時の処理に過ぎない。したがって、再認証を一度も
行わずに、ブラウザの DevTools（Network タブの RSC レスポンス、または React DevTools
の props インスペクタ）から発行済み招待コードの平文を読み取れる。

### 何を防げていて、何を防げていないか

| 脅威 | 防げるか |
| --- | --- |
| ログイン状態のまま離席した端末を、通りすがりの人が操作する | ✅ 防げる |
| 画面共有中に招待コードが映り込む | ✅ 防げる |
| DevTools を開ける者が、再認証せずにコードを読む | ❌ **防げない** |
| 他人の招待コードを読む | ✅ 防げる（RLS `invite_codes_select_own` による。再認証とは無関係） |

### なぜこの状態を受け入れたか

正しく塞ぐには、パスワードをサーバーへ送って検証する方式へ変える必要がある。
これは 8 章の「再認証のパスワードはブラウザ内で完結させる」という選択を捨てることを
意味し、UI と Server Action の作り直しを伴う。

本システムは招待制のクローズドな個人用途であり、想定する脅威は「離席した端末」で
あって「DevTools を操る攻撃者」ではない。認証済みユーザーが読めるのは自分自身が
発行したコードのみで、その保護は RLS が担っている。よって現時点では受け入れる。

### 将来この判断を見直すべき条件

- 招待コード以外の、より機微な情報（決済情報、他人の個人情報など）を
  同じ再認証で守ろうとしたとき
- 不特定多数へ公開する運用に変わったとき

そのときは「発行直後に一度だけ表示し、以後は再取得不可にする」方式
（GitHub の Personal Access Token と同じ）が、実装量と防御力の釣り合いが最も良い。
